...
router.get("/viewGerente", async (req, res) => {
  ...
  let FechaAyerString = FechaAyer.toISOString().split("T")[0];
  ...
  const sqlSelectInv = `SELECT * FROM inventario WHERE DATE(INV_Created_at) = ?`;
  let [resultSelectInv] = await db.promise().query(sqlSelectInv, FormatFechaHoy);
  const sqlSelectInvAyer = `SELECT * FROM inventario WHERE DATE(INV_Created_at) = ?`;
  let [resultSelectInvAyer] = await db.promise().query(sqlSelectInvAyer, FechaAyerString);
  let objProductos = {};
  ...
  resultSelectInv.forEach((row) => {
    objProductos[`${row.INV_Referencia}INV`] = row.INV_Cantidad;
  })
  resultSelectInvAyer.forEach((row) => {
    objProductos[`${row.INV_Referencia}INVAyer`] = row.INV_Cantidad;
  })
